...
typedef boost::property_map < Graph, boost::vertex_name_t >::type vertexMap;
vertexMap vertexName = get(boost::vertex_name, g_Graph);
typedef boost::property_map < Graph, boost::edge_name_t >::type edgeMap;
edgeMap edgeName = get(boost::edge_name, g_Graph);
typedef boost::graph_traits < Graph >::vertex_descriptor bVertex;
...
int main(int argc, char* argv[])
{
	...
	for(int i=0;i<argc;i++)
	{
		...
		else if(!strcmp(argv[i], "-b")) sg_bBoost=true;
	}
