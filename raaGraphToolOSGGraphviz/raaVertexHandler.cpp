...
void raaVertexHandler::pick( osgViewer::View* pView, float x, float y )
{
	...
	if(m_pSelectedSwitch)
	{
		...
		((raaVertexCallback*)m_pSelectedSwitch->getParent(0)->getUpdateCallback())->setActive(false);
		m_pSelectedSwitch=0;
	}
				...
				for(osg::NodePath::const_reverse_iterator rit=it->nodePath.rbegin();rit!=it->nodePath.rend();rit++)
				{
					...
					if(osg::Group *pGroup=dynamic_cast<osg::Group*>(pNode))
					{
						if(pGroup->getName()=="manip")
						{
							...
							((raaVertexCallback*)pGroup->getUpdateCallback())->setActive(true);
							((raaVertexCallback*)pGroup->getUpdateCallback())->setTransform((osg::MatrixTransform*)m_pSelectedSwitch->getChild(m_iManip));
							return;
						}
					}
				}
	...
	{	
		...
		case osgGA::GUIEventAdapter::KEY_Left:
			if(m_pSelectedSwitch)
			{
				...
				((raaVertexCallback*)m_pSelectedSwitch->getParent(0)->getUpdateCallback())->setTransform((osg::MatrixTransform*)m_pSelectedSwitch->getChild(m_iManip));
			}
			break;
...
void raaVertexHandler::updateManipulators()
{
	if(m_pSelectedSwitch)
	{
		osg::MatrixTransform *pMT=(osg::MatrixTransform*)m_pSelectedSwitch->getParent(0)->getChild(0);
		...
		pMT->setMatrix(sm_InvScale*m);

		for(unsigned int i=0;i<m_pSelectedSwitch->getNumChildren();i++)
		{
			pDragger=(osgManipulator::Dragger*)m_pSelectedSwitch->getChild(i);
			pDragger->setMatrix(m);
		}
	}
}
