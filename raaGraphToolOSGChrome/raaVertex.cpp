...
osg::Material *raaVertex::sm_pNormalMaterial=0;
osg::Material *raaVertex::sm_pPinMaterial=0;
osg::ShapeDrawable *raaVertex::sm_pShape=0;
...
{
	if(!sm_pNormalMaterial) sm_pNormalMaterial=makeMaterial(osg::Vec4f(0.2f, 1.0f, 0.3f, 1.0f));
	if(!sm_pPinMaterial) sm_pPinMaterial=makeMaterial(osg::Vec4f(1.0f, 0.2f, 0.3f, 1.0f));
	if(!sm_pShape) sm_pShape=new osg::ShapeDrawable(new osg::Sphere(osg::Vec3f(0.0f, 0.0f, 0.0f), 0.1f));
	...
	pGeode0->getOrCreateStateSet()->setAttributeAndModes(sm_pNormalMaterial, osg::StateAttribute::ON);
	pGeode0->addDrawable(pSD);
	m_pMaterialSwitch->addChild(pGeode0);

	osg::Geode *pGeode1=new osg::Geode();
	pGeode1->getOrCreateStateSet()->setAttributeAndModes(sm_pPinMaterial, osg::StateAttribute::ON);
...
/*	osg::MatrixTransform *pMTP=(osg::MatrixTransform*)m_pGroup->getChild(0);

	osg::Matrixf m;
	m.makeTranslate(m_vPos);
	pMTP->setMatrix(m);
...
void raaVertex::setFixed(bool bFixed)
{
	raaNode::setFixed(bFixed);
