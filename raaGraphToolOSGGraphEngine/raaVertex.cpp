...
std::string raaVertex::csm_sFont="Fonts/Verdana/Verdana Regular.ttf";
...
raaVertex::raaVertex(std::string sID, std::string sName, raaGraph *pGraph) : raaNode(sID, sName, pGraph)
{
	...
	osg::ShapeDrawable *pSD=new osg::ShapeDrawable(new osg::Sphere(osg::Vec3f(0.0f, 0.0f, 0.0f), 0.06f));
	...
	pBB->getOrCreateStateSet()->setAttributeAndModes(makeMaterial(osg::Vec4f(0.9f, 0.8f, 0.2f, 1.0f)), osg::StateAttribute::ON);

	pBB->setMode(osg::Billboard::POINT_ROT_EYE);
	pBB->setAxis(osg::Vec3f(0.0f, 1.0f, 0.0f));
	pBB->setNormal(osg::Vec3f(0.0f, 0.0f, 1.0f));
	pBB->setPosition(0, osg::Vec3f(0.25f, 0.1f, 0.0f));
...
float raaVertex::update()
{
	...
	else
	{
		...
		return m_fEnergy=m_fMass*fabs(m_vVelocity.length());
	}
}
...
bool raaVertex::run(osg::Object* object, osg::Object* data)
